...
# NEW SETTINGS (ADDED)
# ------------------------------------------------------------------
# True  = UTR submit hote hi auto-verify ho kar file automatically deliver ho jayegi.
# False = pehle wala manual system (admin Approve/Reject button dabayega).
AUTO_APPROVE = True

# Payment verify hone par user ko yeh file bheji jayegi.
# Is file ko (e.g. app.apk) bot.py ke saath same folder me rakhna hai.
APK_FILE = "app.apk"

USER_FILE = "users.txt"
USED_UTRS_FILE = "used_utrs.txt"
ORDERS_FILE = "orders.json"  # NEW: saare orders yahan store honge
...
# NEW: Orders store (JSON based) — admin panel ke liye
def get_orders():
...
def build_orders_csv_bytes():
    """Saare orders ka CSV bana ke bytes me return karein (report download ke liye)."""
    orders = get_orders()
    output = io.StringIO()
    writer = csv.writer(output)
    writer.writerow(["User ID", "First Name", "Username", "UTR", "Amount (Rs)", "Time"])
    for o in orders:
        writer.writerow([
            o.get("user_id", ""),
            o.get("first_name", ""),
...
# ------------------------------------------------------------------
# NEW: AUTO DELIVERY + ADMIN NOTIFICATION HELPERS
# ------------------------------------------------------------------
def deliver_access(user, utr):
    """
    Verified user ko file (APK) directly bhejein.
    Agar APK_FILE server par nahi hai to user ko admin se contact karne ka msg milega.
    """
    try:
        ...
        bot.send_message(user.id, "✅ Verification Complete! Enjoy your course access.")
    except FileNotFoundError:
        # File server par abhi update nahi hai — graceful fallback
        bot.send_message(
            user.id,
            f"🎉 Payment Verified (₹{PRICE})! App file abhi server par update ho rahi hai.\n"
            f"Kripya Admin {ADMIN_USERNAME} se contact karein."
        )
    except Exception as e:
        logger.error(f"APK delivery failed for {user.id}: {e}")
        bot.send_message(
            user.id,
            f"🎉 Payment Verified (₹{PRICE})! Delivery me thodi delay hai, Admin {ADMIN_USERNAME} se contact karein."
        )

def notify_admin_new_order(order, auto=True):
    """Har naye order ki jankari Admin ko bhejein (informational)."""
    status = "AUTO-APPROVED" if auto else "PENDING VERIFICATION"
    text = (
        f"🛒 *NEW ORDER — {status}*\n"
        "━━━━━━━━━━━━━━━━━━━━━━\n"
...
def stats_command(message):
    ...
    users = get_users()
    orders = get_orders()
    total_revenue = sum(int(o.get("amount", 0) or 0) for o in orders)
...
def orders_command(message):
    ...
    orders = get_orders()
    if not orders:
        bot.send_message(message.chat.id, "📭 Abhi tak koi order nahi aaya.")
        return

    recent = orders[-10:][::-1]  # last 10 orders, newest first
    lines = [f"🛒 RECENT ORDERS (Total: {len(orders)})", "━━━━━━━━━━━━━━━━━━━━━━"]
    for o in recent:
        lines.append(
            f"👤 {o.get('first_name')} (@{o.get('username') or 'No_Username'})\n"
            f"   🆔 {o.get('user_id')} | ₹{o.get('amount')}\n"
            f"   🔢 UTR: {o.get('utr')} | 🕒 {o.get('time')}"
        )
    lines.append("\n📥 Full list CSV me download karne ke liye: /report")
    bot.send_message(message.chat.id, "\n\n".join(lines))

# NEW: /report — full CSV download
...
# ------------------------------------------------------------------
# 5. PAYMENT & UTR VERIFICATION (AUTO-APPROVE SYSTEM)
# ------------------------------------------------------------------
...
def process_payment(call):
    ...
    caption = (
        ...
        "2. Payment ke baad 12-Digit UTR/Transaction ID strictly verify hogi.\n"
...
def process_utr_submission(message):
    ...
    # 2. Blacklist common fake / suspicious UTR patterns
    fake_patterns = ["000000000000", "123456789012", "111111111111", "999999999999"]
    all_same_digit = re.match(r"^(\d)\1{11}$", utr)  # e.g. 222222222222
    if utr in fake_patterns or all_same_digit:
    ...
    # UTR valid aur unique hai — order save karo + used mark karo
    ...
    if AUTO_APPROVE:
        # ---- AUTO MODE: turant verify + direct delivery ----
        bot.send_message(
            message.chat.id,
            "✅ *Payment Record Received!*\n\n"
            f"🔹 Amount: ₹{PRICE}\n"
            f"🔹 UTR: `{utr}`\n\n"
            "⏳ Verify karke aapki file bheji ja rahi hai...",
            parse_mode="Markdown"
        )
        deliver_access(user, utr)
        notify_admin_new_order(order, auto=True)
    else:
        # ---- MANUAL MODE (pehle wala) ----
        bot.send_message(
            message.chat.id,
...
# ------------------------------------------------------------------
# 6. ADMIN APPROVAL & REJECTION CALLBACKS (sirf AUTO_APPROVE=False par use hote hain)
# ------------------------------------------------------------------
